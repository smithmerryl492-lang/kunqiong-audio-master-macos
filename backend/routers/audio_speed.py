...
async def change_speed(
    ...
    speed: float = Form(1.0),  # 速度倍数 0.5-2.0
    keep_pitch: bool = Form(True),  # 是否保持音调
    ...
    参数:
    - file: 音频文件
    - speed: 速度倍数 (0.5-2.0，1.0为原速)
    - keep_pitch: 是否保持音调（True=变速不变调，False=变速变调）
    - output_format: 输出格式
    - output_path: 输出目录
    """
    if speed < 0.25 or speed > 4.0:
        raise HTTPException(status_code=400, detail="速度倍数必须在 0.25-4.0 之间")
    ...
    try:
        ...
        if keep_pitch:
            ...
            # 变速变调：使用 asetrate + aresample
            filter_str = f"asetrate=44100*{speed},aresample=44100"
        
        cmd = ['ffmpeg', '-y', '-i', input_path, '-af', filter_str, final_output]
        result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8')
        
        if result.returncode != 0:
            raise HTTPException(status_code=500, detail=f"变速失败: {result.stderr}")
        ...
        raise HTTPException(status_code=500, detail=f"变速失败: {str(e)}")
    finally:
