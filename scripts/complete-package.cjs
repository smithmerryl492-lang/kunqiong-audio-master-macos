...
async function main() {
  ...
  logHeader('音频处理大师 v1.0.4 - 完整打包脚本');
  ...
  try {
    ...
    if (fs.existsSync(requirementsPath)) {
      ...
      try {
        execSync('python -m pip check', { stdio: 'pipe' });
        logSuccess('Python 依赖检查通过');
      } catch (e) {
        logWarning('Python 依赖可能有问题，尝试重新安装...');
        if (!runCommand(`python -m pip install -r "${requirementsPath}"`)) {
...
`;
    ...
    console.log();
    logInfo('特点:');
    logInfo('• 完全独立运行，无需安装任何环境');
    logInfo('• 可直接复制到其他 Windows 机器使用');
    logInfo('• 包含完整的错误处理和日志记录');
    logInfo('• 优化的启动速度和资源占用');
